...
function initWebUploader(webpath) {
    ...
    var btnObj = $('<span id="up1"></span>').appendTo(parentObj);
    ...
    uploader.on('error', function (type) {
        switch (type) {
            case 'Q_EXCEED_NUM_LIMIT':
                ...
                dialog({ title: '提示', content: '错误：上传文件数量过多,请重新上传！', okValue: '确定', ok: function () { } }).showModal();
                break;
            case 'Q_EXCEED_SIZE_LIMIT':
               
                dialog({ title: '提示', content: '错误：文件总大小超出限制,请重新上传！', okValue: '确定', ok: function () { } }).showModal();
                break;
            case 'F_EXCEED_SIZE':
                dialog({ title: '提示', content: '错误：文件大小超出限制,请重新上传！', okValue: '确定', ok: function () { } }).showModal();
                break;
            case 'Q_TYPE_DENIED':
              
                dialog({ title: '提示', content: '错误：禁止上传该类型文件,请重新上传！', okValue: '确定', ok: function () { } }).showModal();
                break;
            case 'F_DUPLICATE':
             
                dialog({ title: '提示', content: '错误：请勿重复上传该文件,请重新上传！', okValue: '确定', ok: function () { } }).showModal();
                break;
            default:
                alert('错误代码：' + type);
                dialog({ title: '提示', content:'错误代码：'+type+'', okValue: '确定', ok: function () { } }).showModal();
                break;
        }
    });
    //当有文件添加进来的时候
    ...
    uploader.on('uploadSuccess', function (file, data) {
        ...
        $("#txtShangchaunimg").val(data.path);
